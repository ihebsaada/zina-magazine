...
export async function addComment(
  ...
  if (!articleId || !authorName || !commentBody) {
    return { error: t('articleId, authorName and body are required', 'معرف المقال، اسم الكاتب والمحتوى مطلوبون') }
  }

  if (commentBody.length > 500) {
    return { error: t('Comment exceeds 500 characters', 'تجاوز التعليق الحد الأقصى 500 حرف') }
  }

  if (authorName.length > 80) {
    return { error: t('Author name exceeds 80 characters', 'تجاوز اسم الكاتب الحد الأقصى 80 حرف') }
  }

  if (isSpam(commentBody) || isSpam(authorName)) {
    // Message volontairement direct pour le spam
    return { error: t('Comment flagged as spam', 'تم تمييز التعليق كبريد عشوائي') }
  }
