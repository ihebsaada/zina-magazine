...
export function LikeWidget({ articleId, locale, dict, initialLikeCount }: LikeWidgetProps) {
  ...
  // ── Résolution de session asynchrone (LocalStorage) ───────────────────────
  // Impossible en SSR, on interroge l'API uniquement pour le booléen `isLiked`
  useEffect(() => {
    ...
    fetch(`/api/likes?articleId=${articleId}&sessionId=${sessionId}`)
      ...
      .then(data => {
        ...
        // Note: Le count a déjà été hydraté par le SSR de manière immédiate !
      })
      .catch(() => {})
  }, [articleId])

  // ── Action (Server Action Optimiste) ──────────────────────────────────────
  ...
  return (
    ...
    >
      ...
      <span className="sr-only sm:not-sr-only text-sm font-medium">{t.likes}</span>
    </button>
  )
}
