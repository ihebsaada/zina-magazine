...
const components: PortableTextComponents = {
  block: {
    normal: ({ children }) => (
      <p className="mb-6 leading-relaxed text-[var(--color-ink-800)]">
        {children}
      </p>
    ),
    h2: ({ children }) => (
  ...
  marks: {
    strong: ({ children }) => (
      <strong className="font-semibold text-[var(--color-ink-950)]">
        {children}
      </strong>
    ),
  ...
  list: {
    bullet: ({ children }) => (
      <ul className="list-disc list-inside mb-6 space-y-2 text-[var(--color-ink-800)]">
        {children}
      </ul>
    ),
    number: ({ children }) => (
      <ol className="list-decimal list-inside mb-6 space-y-2 text-[var(--color-ink-800)]">
        {children}
      </ol>
    ),
  },
...
export function ArticleBody({ body, className }: ArticleBodyProps) {
  if (!body || body.length === 0) {
    return (
      <div
        className={cn(
          "prose-editorial text-[var(--color-ink-600)] italic",
          className,
        )}
      >
