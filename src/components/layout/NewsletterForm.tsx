...
export function NewsletterForm({ locale, dict, className }: NewsletterFormProps) {
  ...
  const t = {
    title      : dict?.newsletter?.title       ?? (isRTL ? 'اشترك في نشرتنا' : 'Subscribe to our newsletter'),
    subtitle   : dict?.newsletter?.subtitle    ?? (isRTL ? 'قصص تهمك، أسبوعياً.' : 'Stories that matter. Delivered weekly.'),
    placeholder: dict?.newsletter?.placeholder ?? (isRTL ? 'بريدك الإلكتروني' : 'your@email.com'),
    cta        : dict?.newsletter?.cta         ?? (isRTL ? 'اشترك' : 'Subscribe'),
    success    : dict?.newsletter?.success     ?? (isRTL ? 'شكراً! تم تسجيلك بنجاح.' : 'Thank you! You&apos;re subscribed.'),
    duplicate  : dict?.newsletter?.duplicate   ?? (isRTL ? 'أنت مشترك بالفعل.' : 'You&apos;re already subscribed.'),
    error      : dict?.newsletter?.error       ?? (isRTL ? 'حدث خطأ. حاول مجدداً.' : 'Something went wrong. Please retry.'),
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (result.success) {
        ...
        setMessage(result.message?.includes('déjà inscrit') ? t.duplicate : t.success)
        setEmail('')
      } else {
        setStatus('error')
  ...
  return (
    <div className={className} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {status === 'error' && (
        <p className="mt-3 text-sm text-[var(--color-blush)] text-center">{message}</p>
      )}
    </div>
  )
}
